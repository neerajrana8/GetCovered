  ...
  class SendPoliciesListJob < ApplicationJob
    ...
    def generate_csv(policies)
      csv_string = CSV.generate(headers: true) do |csv|
        csv << ['Number', 'Status', 'T Code', 'Community', 'Building', 'Unit', 'PM Account', 'Agency', 'Effective Date',
        ...
        policies.each do |policy|
          csv << [policy.dig('number'), policy.dig('status'), policy.dig('tcode'), policy.dig('primary_insurable', 'parent_community', 'title'),
                  ...
                  policy.dig('account', 'title'), policy.dig('agency', 'title'), policy.dig('effective_date')&.to_date&.strftime("%B %e, %Y"),
                  policy.dig('expiration_date')&.to_date&.strftime("%B %e, %Y"), policy.dig('primary_user', 'full_name'),
                  (policy.dig('primary_user', 'email') || policy.dig('primary_user', 'contact_email')),
                  policy.dig('policy_type_title'), policy.dig('billing_strategy'), policy.dig('updated_at')&.to_date&.strftime("%B %e, %Y"),
