    ...
    ids&.each do |integration_id|
      ...
      begin
        integration.configuration['sync']['syncable_communities'].select do |property_id, config|
          ...
          next if !integration.configuration['sync']['queued_resync'] && config['last_sync_f'] == Time.current.to_date.to_s && config['last_sync_i'] == Time.current.to_date.to_s
      ...
      if integration.configuration['sync']['push_policies']
        ...
        begin
          integration.configuration['sync']['syncable_communities'].select do |property_id, config|
            next unless config['enabled']
            next if !integration.configuration['sync']['queued_resync'] && config['last_sync_p'] == Time.current.to_date.to_s
