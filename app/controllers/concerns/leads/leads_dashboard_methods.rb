module Leads
  # Move logic to Model or Library cause this is not Rails Concern
      ...
      if stats.nil?
        ...
        if filter
          ...
          if filter[:lead_events].present?
            ...
            leads_by_policy_type = Lead.select(:id).distinct(:id).presented.not_converted
        ...
        unless total_by_status.nil?
          ...
          leads.each do |s|
            next if s.lead_events_timeseries.nil?

            s.lead_events_timeseries.each do |k, v|
              td = Time.zone.parse(k).strftime(date_slug_format)
              stats_by[td][:site_visits] = v if stats_by[td].present?
            end
          end
