module BrandingProfilesMethods
  ...
  def attach_images
    if update_allowed?
      ...
      second_footer_logo_status = process_image(:second_footer_logo_url) if attach_images_params[:second_footer_logo_url].present?
      ...
      if logo_status == 'error' || logo_jpeg_status == 'error' || footer_status == 'error' || second_logo_status == 'error'
        ...
        render json: {
                 ...
                 logo_jpeg_url: logo_jpeg_status,
                 footer_logo_url: footer_status,
                 second_logo_url: second_logo_status,
                 second_footer_logo_url: second_footer_logo_status
               }, status: :ok
      end
    else
  ...
  def second_footer_logo_delete
    if update_allowed?
      if @branding_profile.update(second_footer_logo_url: nil)
        render json: { message: 'Second footer logo deleted' }, status: :ok
      else
        render json: { message: @branding_profile.errors }, status: :unprocessable_entity
      end
    else
  ...
  def attach_images_params
    ...
    params.require(:images).permit(:logo_url, :logo_jpeg_url, :footer_logo_url, :second_logo_url, :second_footer_logo_url)
  end

  def validate_images_size
    images = %i[logo_url logo_jpeg_url footer_logo_url second_logo_url second_footer_logo_url]
