    ...
    class TrackingUrlsController < StaffAgencyController
      ...
      def agency_filters
        ...
        result << if sub_agencies.any?
                    sub_agencies_attr = sub_agencies.map{|el| el.attributes.merge("branding_profiles"=> el.branding_profiles.map{|bp| {id: bp.id, url: bp.url}})}
                    agency_attr = agency.attributes.reverse_merge("agencies"=> sub_agencies_attr)
                    agency_attr.merge("branding_profiles"=> agency.branding_profiles.map{|bp| {id: bp.id, url: bp.url}})
                  else
