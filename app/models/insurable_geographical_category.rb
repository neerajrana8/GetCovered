...
class InsurableGeographicalCategory < ApplicationRecord
  ...
  before_update :implement_cutoffs,
    if: Proc.new{|igc| igc.will_save_change_to_attribute?('irc_cutoffs') }
  ...
  def implement_cutoffs
    # takes autogenerated IRCs (i.e. configurer QBE, configurable_type Insurable) and ensures that none bridge any new cutoff dates
    cpt_id = 1 # only CarrierPolicyType 1, i.e. QBE HO4
    cutoffs = attribute_change_to_be_saved('irc_cutoffs').map{|x| [x[0][cpt_id.to_s], x[1][cpt_id.to_s]] }
