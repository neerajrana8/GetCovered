  ...
  module ConsolidatedReports
    class Base < ::Report
      ...
      def to_csv
        CSV.generate do |csv|
          csv << ['Aggregate']
          to_csv_report(csv, data['aggregate'])

          csv << ['By product']
          csv << []
          data['by_products'].each do |row|
            csv << [PolicyType.find(row['policy_type_id']).title]
            to_csv_report(csv, row['report'])
          end
        end
      end

      def to_csv_report(csv, source)

        csv << ['Site visits', source['site_visits']]
        csv << []
        csv << ['Completed applications']
        csv << ['Conversion Rate', source['completed_applications']['conversion_rate']]
        csv << ['Average Conversion Time', source['completed_applications']['average_time_minutes']]
        csv << []
        csv << ['Number of Leads']
        csv << ['Average # of Visits / Lead', source['leads']['average_leads_visits']]
        csv << ['Count', source['leads']['average_leads_visits']]
        csv << []
        csv << ['Premium']
        csv << ['Total Premium Collected', source['premium']['total']]
        csv << ['Average Premium by Lead', source['premium']['average_lead_premium']]
        csv << ['Average Premium by Policy', source['premium']['average_policy_premium']]
        csv << []
        csv << ['Quotes Generated']
        csv << ['Average Quote Price', source['quotes']['average_quote_price']]
        csv << ['Average Time from Quote to Conversion', source['quotes']['average_time_from_quote_to_conversion']]
        csv << []
        csv << []
      end
