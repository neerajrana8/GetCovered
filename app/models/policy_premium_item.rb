class PolicyPremiumItem < ApplicationRecord
  ...
  # Useful constants
  ProrationCalculationChargeOrder = [   # the order in which to charges apply to line items, based on proration_calculation (refunds occur in reverse)
  ...
  enum proration_calculation: {
    ...
    per_payment_term: 1,
    per_total: 2,
    payment_term_exclusive: 3,
    payment_term_inclusive: 4
  }
    ...
    case self.rounding_error_distribution
      when 'dynamic_forward', 'dynamic_reverse'
        ...
        to_return = to_return.send(reversal).each do |pt|
          pt.original_total_due = ((pt.weight / weight_left) * total_left).floor
          total_left -= pt.original_total_due
      ...
      when 'first_payment_simple', 'last_payment_simple', 'first_payment_multipass', 'last_payment_multipass'
        ...
        loop do
          ...
          to_return.each do |pt|
            ...
            pt.original_total_due += li_total
        ...
        to_return.send(reversal).find{|tp| tp.original_total_due > 0 }.original_total_due += to_distribute
    end
    # save changes
    to_return.each{|tr| tr.save }
    # return line items MOOSE WARNING: save these instead, no?
    return to_return.map do |pt|
      pt.original_total_due == 0 ? nil : LineItem.new(
        chargeable: pt,
        title: self.title,
