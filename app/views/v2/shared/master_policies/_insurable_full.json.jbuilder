...
json.active_master_policy do
  if insurable.policies.current.where(policy_type_id: [PolicyType.master.ids, PolicyType::MASTER_COVERAGE_ID]).any?
    json.partial! 'v2/shared/policies/fields.json.jbuilder',
                  policy: insurable.policies.current.where(policy_type_id: [PolicyType.master.ids, PolicyType::MASTER_COVERAGE_ID]).take
  end
end
