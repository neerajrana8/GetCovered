...
json.active_master_policy do
  if insurable.policies.current.where(policy_type_id: [PolicyType::MASTER_IDS, PolicyType::MASTER_COVERAGE_ID]).any?
    json.partial! 'v2/shared/policies/fields.json.jbuilder',
                  policy: insurable.policies.current.where(policy_type_id: [PolicyType::MASTER_IDS, PolicyType::MASTER_COVERAGE_ID]).take
  end
end
