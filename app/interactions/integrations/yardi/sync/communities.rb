        ...
        def execute
          ...
          unless do_sync_units == false
            ...
            property_results.each do |pr|
              ...
              # sync dem units
              pr[:unit_sync] = Integrations::Yardi::Sync::Units.run!(integration: integration, community: pr[:community], community_address: pr[:yardi_property_data], property_id: pr[:integration_profile].external_id, do_sync_tenants: do_sync_tenants, tenant_array: tenant_array)
            end
            unless tenant_array.blank?
              tenant_results = Integrations::Yardi::Sync::Tenants.run!(integration: integration, tenant_array: tenant_array)
            end
          end
          # done
