class UpgradePolicyCancellationColumns < ActiveRecord::Migration[5.2]
  def up
    ...
    Policy.where.not(cancellation_code: nil).each do |pol|
      if pol.cancellation_code == 'TEST'
        pol.update_columns(cancellation_reason: 'test_policy')
      else
        case pol.carrier_id
          when 1,2,4 # WARNING: we're using QBE codes for pensio here
  ...
  def down
    ...
    Policy.where.not(cancellation_reason: nil).each do |pol|
      if pol.cancellation_reason == 'test_policy'
        pol.update_columns(cancellation_code: 'TEST')
      else
        case pol.carrier_id
          when 1,2,4 # WARNING: we're using QBE codes for pensio here
