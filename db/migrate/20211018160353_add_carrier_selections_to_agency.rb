  ...
  def change
    ...
    CarrierAgencyPolicyType.references(:carrier_agencies).includes(:carrier_agency).where(available: true).each do |capt|
      ...
      if capt.policy_type_id == ::PolicyType::RESIDENTIAL_ID && capt.carrier_agency.carrier_id == 5 # residential defaults to MSI wherever possible
        agency.carrier_selections['by_policy_type'][capt.policy_type_id.to_s][capt.state].unshift(capt.carrier_agency.carrier_id)
      else
        agency.carrier_selections['by_policy_type'][capt.policy_type_id.to_s][capt.state].push(capt.carrier_agency.carrier_id)
      end
    end
