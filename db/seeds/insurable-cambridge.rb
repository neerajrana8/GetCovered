...
if Account.exists?(title: "Occupant Shield")
	@occupant_shild = Account.where(title: "Occupant Shild").take
else
...
@cambridge_communities = [ 
  Insurable.new(title: "Five Vanderbilt",
  ...
  Insurable.new(title: "Three Vanderbilt",
								insurable_type: InsurableType.find(1), 
								enabled: true, category: 'property',
								account: @occupant_shield,
								addresses_attributes: [
  ...
  Insurable.new(title: "Four Executive",
								insurable_type: InsurableType.find(1), 
								enabled: true, category: 'property',
								account: @occupant_shield,
								addresses_attributes: [
  ...
  Insurable.new(title: "Two Pan American",
								insurable_type: InsurableType.find(1), 
								enabled: true, category: 'property',
								account: @occupant_shield,
								addresses_attributes: [
  ...
  Insurable.new(title: "One Pan American",
								insurable_type: InsurableType.find(1), 
								enabled: true, category: 'property',
								account: @occupant_shield,
								addresses_attributes: [
...
@cambridge_communities.each_with_index do |community, index|
	if community.save
		...
		Assignment.create!(staff: @occupant_shield.owner, assignable: community)
		...
		if index == 0
			# Five Vanderbilt
			unit_list = [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 201, 
			...
			# Three Vanderbilt
			unit_list = [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 
		...
		unless unit_list.nil?
			unit_list.each do |unit|				 
				unit = community.insurables.new(title: unit, insurable_type: InsurableType.find(4),
		...
		if community.units.count > 0
			community.reset_qbe_rates(true, true)
		  community.insurable_rates.optional.update_all mandatory: true
		  
		  enabled_cov_c_rates_ids = []
		  enabled_cov_c_rates_ids.concat community.insurable_rates.coverage_c.where("(coverage_limits ->> 'coverage_c')::integer = ?", 1000000).map(&:id)
		  enabled_cov_c_rates_ids.concat community.insurable_rates.coverage_c.where("(coverage_limits ->> 'coverage_c')::integer = ?", 1500000).map(&:id)
		  enabled_cov_c_rates_ids.concat community.insurable_rates.coverage_c.where("(coverage_limits ->> 'coverage_c')::integer = ?", 2000000).map(&:id)
		  
		  query = "(coverage_limits ->> 'liability')::integer >= ? AND (coverage_limits ->> 'medical')::integer = ?"
		  
		  enabled_liability_rates_ids = community.insurable_rates.liability.where(query, 10000000, 100000).map(&:id)
		  
		  community.insurable_rates.coverage_c.where.not(id: enabled_cov_c_rates_ids).update_all enabled: false
		  community.insurable_rates.liability.where.not(id: enabled_liability_rates_ids).update_all enabled: false		
		end
	else
		pp community.errors
	end	
